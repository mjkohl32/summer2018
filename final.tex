...
The first run of the VAE used no labels and included all the blips from both detectors during O2. Unsurprisingly, there are so many dots on the scatter plot that the graph is essentially a huge blob. However, labeling each blip with its interferometer distinguishes the blips slightly more, as seen in Figure \ref{fig:unlabeled}. Interestingly, there does appear to be a group offset from the rest in both plots.
...
\begin{figure}[h!]
	...
	\end{subfigure}
	\caption{Initial VAE scatter plots of all blips.}
	\label{fig:unlabeled}
\end{figure}

Blah blah blah something about the top plots blah blah blah something about the lower plots

\begin{figure}[h!]
	\centering
	\begin{subfigure}{.8\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{vae_llo_freq_labeled}
		\caption{Livingston blips labeled by their bin in Figure \ref{fig:llo_centr}.}
		\label{fig:llo_freq_labeled}
	\end{subfigure}
	\begin{subfigure}{.8\textwidth}
		\centering
		\includegraphics[width=1\textwidth]{vae_lho_freq_labeled}
		\caption{Hanford blips labeled by their bin in Figure \ref{fig:lho_centr}.}
		\label{fig:lho_freq_labeled}
	\end{subfigure}
	\caption{Blips labeled by frequency bins.}
	\label{fig:freq_labeled}
\end{figure}

Blah blah blah something about the frequency plots blah blah blah something about the subclass plots

\begin{figure}[h!]
	\centering
	\begin{subfigure}{.8\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{vae_llo_labeled}
		\caption{Livingston blips labeled by subclass.}
		\label{fig:llo_labeled}
	\end{subfigure}
	\begin{subfigure}{.8\textwidth}
		\centering
		\includegraphics[width=1\textwidth]{vae_lho_labeled}
		\caption{Hanford blips labeled by subclass.}
		\label{fig:lho_labeled}
	\end{subfigure}
	\caption{Blips labeled by subclass.}
	\label{fig:vae_labeled}
\end{figure}

Blah blah blah something about the suclasses slash the sticks and such blah blah blah something about conclusions and such blah blah blah
